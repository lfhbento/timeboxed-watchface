...
class Layout extends Component {
  constructor(props) {
    ...
    let newState = this.filterValidKeys(this.props.state, [
      ...this.colorKeys,
    ...
    this.textModules =
      this.platform === "aplite" ? this.textModulesAplite : this.textModulesAll;
    this.modules =
      this.platform === "aplite" ? this.modulesAplite : this.modulesAll;
  }
    ...
    return Object.keys(localStorage).reduce((presets, key) => {
      if (key.indexOf("preset-") === 0) {
        ...
        newPreset = Object.keys(newPreset).reduce((preset, key) => {
          ...
          value =
            typeof value === "string" && value.indexOf("0x") !== -1
              ? value.replace("0x", "#")
              : value;
  ...
  isEnabled = moduleIndexes => {
    return (
      this.moduleStateKeys.some(
        key => moduleIndexes.indexOf(this.state[key]) !== -1
      ) ||
      (this.state.showSleep &&
        this.moduleSleepStateKeys.some(
          key => moduleIndexes.indexOf(this.state[key]) !== -1
        )) ||
      (this.state.showTap &&
        this.moduleTapStateKeys.some(
          key => moduleIndexes.indexOf(this.state[key]) !== -1
        )) ||
      (this.state.showWrist &&
        this.moduleWristStateKeys.some(
          key => moduleIndexes.indexOf(this.state[key]) !== -1
        ))
    );
  };
  ...
  getMasterKeyData = () => {
    ...
    fetchMasterKeyData(
      this.state.masterKeyEmail,
      this.state.masterKeyPin,
      keys => {
    ...
    return (
      <div>
            ...
            <div>
              ...
              {this.isEnabled(["17"]) && (
                <ColorPicker
                  ...
                  onSecondColorChange={this.onChange.bind(
                    this,
                    "batteryLowColor"
                  )}
                />
              ...
              {this.isEnabled(["14"]) && (
                <div>
                  ...
                  <HelperText>
                    {this._(
                      "Set upper and lower limits in the health section below."
                    )}
                  </HelperText>
                </div>
              )}
        ...
        <div className="block--footer">
          <HelperText standalone={true}>
            {this._("Remember to save to apply your settings.")}
          </HelperText>
          <HelperText standalone={true}>
            {this._(
